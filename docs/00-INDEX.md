...
7. Intégration comptable *(décision du 10/07/2026)* : la connexion Pennylane (prévue V2/Phase 3, doc 02 l.72) est **avancée à maintenant** en Phase A (token API collé, lecture seule) — ajout assumé sans retrait compensatoire. Périmètre : factures impayées importées → **dossier en 1 clic** (PDF joint, valeurs sourcées et éditables) + **détection de paiement** par polling des changelogs → suggestion de clôture (jamais automatique). Rattachement pricing : plan Pro (aimant à activation) — voir doc 15 pour la recherche complète, doc 06 pour le modèle (`org_integrations`, `accounting_invoices`). Phase B (OAuth « Se connecter avec Pennylane ») : demande de partenariat à envoyer.
