...
type State = "idle" | "recording" | "processing" | "ready" | "manual" | "error";
...
/*
 * Récit vocal → transcription réelle. Enregistre (MediaRecorder), dépose l'audio
 * en direct dans le Storage (URL signée), puis Nora transcrit côté serveur
 * (Groq/Deepgram/OpenAI selon la clé du coffre). Le transcript est éditable et
 * validé par l'utilisateur (sa version fait foi). Sans clé STT → saisie texte.
 */
export function VoiceCapture({
  ...
  function stop() {
    recRef.current?.stop(); // déclenche onstop → process()
    setState("processing");
  }
  ...
  async function process(blob: Blob) {
    try {
      ...
      if (prep.error || !prep.path || !prep.token) {
        setNote("Envoi impossible. Vous pouvez écrire votre récit ci-dessous.");
      ...
      if (upErr) {
        setNote("Envoi de l’audio impossible. Vous pouvez écrire votre récit ci-dessous.");
        return setState("manual");
      }
      const res = await transcribeVoice(prep.path);
      if (res.transcript) {
        setStory(res.transcript);
        setState("ready");
        return;
      }
      if (res.error === "no_provider") {
        setNote("La transcription n’est pas encore activée. Écrivez votre récit ci-dessous (ou l’équipe branche une clé STT).");
      } else {
        setNote("La transcription a échoué. Réessayez, ou écrivez votre récit.");
      }
      setState("manual");
    } catch {
      setNote("Une erreur est survenue. Écrivez votre récit ci-dessous.");
      setState("manual");
    }
  }
  ...
  // ── processing : Nora transcrit ──────────────────────────────────────────────
  if (state === "processing") {
    return (
      <div className="flex items-center gap-4 rounded-[1.5rem] border bg-card p-6">
        ...
        <div className="min-w-0">
          <p className="flex items-center gap-2 font-medium">
            Nora transcrit votre récit
            <span className="flex gap-1">
              {[0, 1, 2].map((i) => (
                <span key={i} className="size-1.5 animate-bounce rounded-full bg-brand" style={{ animationDelay: `${i * 0.15}s` }} />
              ))}
            </span>
          </p>
          <p className="mt-0.5 text-sm text-muted-foreground">Envoi de l’audio, puis transcription — quelques secondes.</p>
        </div>
      </div>
    );
  }

  // ── ready : transcript éditable / manual : saisie ────────────────────────────
  return (
